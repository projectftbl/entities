...
export default function entities(options) {
  return (state = {}, action) => {
    if (action.type === SIGN_OUT_SUCCESS) {
      return options == null ? {} : _.pick(state, options);
    }
...
export const entitiesReducer = ([ FETCH, SUCCESS, FAILED, CLEAR ], options = { clearOnSignOut: true }) => {
  ...
  return (state = initialState, action) => {
    ...
    case SIGN_OUT_SUCCESS:
      return options.clearOnSignOut ? initialState : state;
